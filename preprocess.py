...
def compute_lyrics_embedding(text, mel_T, device='cpu'):
	...
	emb_rep = np.tile(emb[None, :], (mel_T, 1)).astype(np.float32)
	return emb_rep


def build_conditional_feats(smplfull, sliced, start_seconds, mel_T, sequence_seconds=6, fps=30, vocab_size=512):
	"""Return per-frame motion (poses + Th + Rh), lyrics embedding (T, emb_dim),
	plus lyric_text and frame start/end.
	...
	for i in range(total_frames):
		...
		if k in smplfull:
			...
			if len(frames) > 0:
				...
				zero_poses = np.zeros((72,), dtype=np.float32)
	...
	for k in keys:
		try:
			...
			if ':' in k:
				minutes_part, seconds_part = k.split(':')
				val = float(minutes_part) * 60.0 + float(seconds_part)
			else:
				val = float(k)
		except Exception:
			val = 0.0
	...
	if found_key is None and len(keys) > 0:
		...
		for k in keys:
			try:
				if ':' in k:
					minutes_part, seconds_part = k.split(':')
					t = float(minutes_part) * 60.0 + float(seconds_part)
				else:
					t = float(k)
			except Exception:
				t = 0.0
	...
	if found_key is not None:
		lyric_text = sliced.get(found_key, '')
		# compute roberta embedding and repeat to mel_T
		lyric_vec = compute_lyrics_embedding(lyric_text, mel_T)
	else:
		# fallback to zeros bag-of-words-like vector
		lyric_vec = np.zeros((mel_T, 768), dtype=np.float32)

	frame_start = start_frame
	frame_end = start_frame + total_frames
...
def make_dataset(root_in, out_dir, sequence_seconds=6, fps=30, sr=22050, use_bigvgan=True, hparams=None, device='cpu'):
	...
	count = 0
	# We'll perform two passes: 1) write augmented npz and
	...
	for year in years:
		for song in os.listdir(year):
			...
			for k in list(sliced.keys()):
				try:
					minutes_part, seconds_part = k.split(':')
					start_seconds = float(minutes_part) * 60.0 + float(seconds_part)
				...
				mel_T = mel.shape[1]
				motion, lyrics_emb, lyric_text, frame_start, frame_end = build_conditional_feats(smplfull, sliced, start_seconds, mel_T, sequence_seconds=sequence_seconds, fps=fps)
				out_name = f"sample_{count:08d}.npz"
				out_path = os.path.join(out_dir, out_name)
				song_name = os.path.basename(song_path)
				time_end = start_seconds + sequence_seconds
				# save augmented (not yet normalized) and include metadata
				np.savez_compressed(
					out_path,
					mel=mel,
					motion=motion,
				...
				# save sample info for JSON list
				sample_info = {
					"npz_name": out_name,
					"song_name": song_name,
					...
					"mel_shape": mel.shape,
					"lyrics_emb_shape": lyrics_emb.shape,
	...
	for f in files:
		...
		try:
			...
			song_name_bytes = d.get('song_name', np.bytes_(""))
			song_name = song_name_bytes.tobytes().decode('utf-8') if hasattr(song_name_bytes, 'tobytes') and song_name_bytes.size > 0 else song_name_bytes.decode('utf-8') if isinstance(song_name_bytes, bytes) else ""
			time_start = d.get('time_start', np.float32(0.0))
			time_end = d.get('time_end', np.float32(0.0))
			lyric_text_bytes = d.get('lyric_text', np.bytes_(""))
			lyric_text = lyric_text_bytes.tobytes().decode('utf-8') if hasattr(lyric_text_bytes, 'tobytes') and lyric_text_bytes.size > 0 else lyric_text_bytes.decode('utf-8') if isinstance(lyric_text_bytes, bytes) else ""
	...
	# save sample info list as JSON (utf-8)
	try:
		list_path = os.path.join(out_dir, "sample_info_list.json")
		with open(list_path, 'w', encoding='utf-8') as f:
			json.dump(sample_list, f, ensure_ascii=False, indent=2)
		print(f"wrote sample list to {list_path}")
	except Exception as e:
		print('warning writing sample_info_list.json', e)
...
if __name__ == '__main__':
	...
	p.add_argument('--root', default=r'/lm2d/dataset')  # your dataset path
	p.add_argument('--out', default=r'/lm2d/npz')	  # your output path
